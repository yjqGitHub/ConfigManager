...
namespace JQ.DataAccess.Repositories
{
    /// <summary>
    ...
    public class BaseDataRepository<T> : IBaseDataRepository<T> where T : class, new()
    {
        ...
        public object InsertOne(T info, string[] ignoreFields = null, bool isIdentity = true)
        {
            SqlQuery query = SqlQueryUtil.BuilderInsertOneSqlQuery(info, TableName, ignoreFields: ignoreFields, isIdentity: isIdentity, dbType: DataType);
            if (isIdentity)
            {
        ...
        /// <param name="isIdentity">是否自增</param>
        /// <returns>如果是自增则返回自增值，不是返回新增行数</returns>
        public async Task<object> InsertOneAsync(T info, string[] ignoreFields = null, bool isIdentity = true)
        {
            SqlQuery query = SqlQueryUtil.BuilderInsertOneSqlQuery(info, TableName, ignoreFields: ignoreFields, isIdentity: isIdentity, dbType: DataType);
            if (isIdentity)
            {
        ...
        /// <param name="ignoreFields">要忽略的字段</param>
        /// <param name="isIdentity">是否自增</param>
        public void InsertMany(List<T> infoList, string[] ignoreFields = null, bool isIdentity = true)
        {
            SqlQuery query = SqlQueryUtil.BuilderInsertManySqlQuery(infoList, TableName, ignoreFields: ignoreFields, isIdentity: isIdentity, dbType: DataType);
        ...
        /// <param name="isIdentity">是否自增</param>
        /// <returns></returns>
        public Task InsertManyAsync(List<T> infoList, string[] ignoreFields = null, bool isIdentity = true)
        {
            SqlQuery query = SqlQueryUtil.BuilderInsertManySqlQuery(infoList, TableName, ignoreFields: ignoreFields, isIdentity: isIdentity, dbType: DataType);
